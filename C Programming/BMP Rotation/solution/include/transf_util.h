...
/*
 * Point with double components
 */
struct dpoint {
  double x, y;
};
/*
 * Point with integer components. More precise than dpoint for ints.s.
 */
struct ipoint {
  int64_t x, y;
};
/*
 * Point with pixel-grid components (center in upper-left corner).
 */
struct gpoint {
  int64_t row, col;
};
 ...
 * Center point
 */
enum cpoint_coords_types { IS_REAL, IS_IMAGINERY };
