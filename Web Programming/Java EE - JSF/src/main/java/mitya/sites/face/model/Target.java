...
import mitya.sites.face.beans.Shot;
...
public final class Target {
    ...
    public ShotRecord processShot(Shot shot, Date timestamp, User user) throws IllegalArgumentException{
        if(!validateCoords(shot)) throw new IllegalArgumentException("Illegal coordinates provided for the target!");
        return new ShotRecord(shot.getX(), shot.getY(), shot.getRadius(), isHit(shot), timestamp, user);
    }

    public ShotRecord processShot(Shot shot, User user) throws IllegalArgumentException{
        return processShot(shot, new Date(), user);
    }
    private static boolean isHit(Shot shot){
        //Rectangle Check
    ...
    private boolean validateCoords(Shot shot) {
