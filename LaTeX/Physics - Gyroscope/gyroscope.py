...
MEASURES_PER_INERTIA = 5

GYROSCOPE_MASS = 1.5 #kg
GYROSCOPE_RADIUS = 12.5/100#meters
LEVER_LENGTH = 22.5/100 #meters

#STUDENT_COEFF = 4.30

G = 9.82
LOAD_HOLDER_MASS = 0.004#kilo
LOAD_MASS = 0.05 #kilo
...
circ_freq1, circ_freq2, period =  np.loadtxt('gyroscope.csv', delimiter=',', dtype=float, skiprows=1, unpack=True)
...
f.write("\hline \n")
f.write( f"$m$, г & $\\omega_1$, об./мин & $\\omega_2$, об./мин & $\\omega_{{\\text{{ср}}}}$, об./мин & $T_\\text{{пр}}$, с\\\\ \n")
f.write("\hline \n")
for i in range(3):
    f.write( f"\\multirow{{5}}{{*}}{{$m_0$ + {i+1} $\cdot$ $m_1$}} & {circ_freq1[i*5+0]:.2f} & {circ_freq2[i*5+0]:.2f} & {circ_freq[i*5+0]*60/(2*math.pi):.2f} & {period[i*5+0]:.2f}\\\\ \n")
    f.write(f"\\cline{{2-5}} \n")
    for j in range(1, 5):
        f.write( f" & {circ_freq1[i*5+j]:.2f} & {circ_freq2[i*5+j]:.2f} & {circ_freq[i*5+j]*60/(2*math.pi):.2f} & {period[i*5+j]:.2f}\\\\ \n")
        f.write(f"\\cline{{2-5}} \n")
    f.write("\hline \n")
